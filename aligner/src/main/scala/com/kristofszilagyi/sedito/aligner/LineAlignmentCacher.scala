...
import com.kristofszilagyi.sedito.aligner.MetricCalculator.PairwiseMetrics
  ...
  def calcLineMetrics(leftIdx: LineIdx, rightIdx: LineIdx): PairwiseMetrics = {
    ...
    cache.get((leftLine, rightLine)) match {
      case Some(metrics) =>
        metrics
      case None =>
        val metrics = MetricCalculator.calcMetrics(leftLine, rightLine, math.max(leftLine.length, rightLine.length))
