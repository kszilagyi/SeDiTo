...
final class WordAlignmentFromOldTest extends FreeSpecLike{
  ...
  "simple changes" in {
    ...
    AmbiguousWordAlignment.fromOld(Lines(Vector(leftLine)), Lines(Vector(rightLine)), alignment).matches.toSeq.sortBy(_.left.from.i) shouldBe
      Set(
  ...
  "ambiguous simple" in {
    ...
    AmbiguousWordAlignment.fromOld(Lines(Vector(leftLine)), Lines(Vector(rightLine)), alignment).matches should (
      equal(Set(
